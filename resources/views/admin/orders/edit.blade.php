                ...
                <div class="table-responsive">
                    <form action="/admin/orders/{{$orders->id}}" method="POST" role="form">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                            <tr>
                                <td style="text-align: center;vertical-align: middle;line-height:40px;">{{old('name',$orders->id)}}</td>
                                <td style="text-align: center;vertical-align: middle;line-height:40px;">{{old('name',$orders->date)}}</td>
                                <td style="text-align: center;vertical-align: middle;line-height:40px;">{{old('name',$orders->sum)}}</td>
                        ...
                        @foreach($item as $items)
                            @foreach($product as $products)
                                @if($items->products_id == $products->id)
                                    <label>{{$products->name}} / 購買數量：{{$items->quantity}}</label>&nbsp;<br>
                                @endif
                            @endforeach
                        @endforeach
                    </form>
